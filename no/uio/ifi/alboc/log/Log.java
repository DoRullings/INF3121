...
public class Log {
    ...
    private static void writeParserData (String data) {
        String str = "Parser:    ";
        for (int i = 0; i<parseLevel; i++) {
            str += "    ";
        }
        writeLogLine(str += data);
    }
	...
	if (! doLogParser && ! doLogScanner) return;
        String ln = String.format("%4d", lineNum);
	writeLogLine(ln + ": " + line); 
    }
	...
	if (Scanner.nextName != null && Scanner.nextToken == nameToken)
	    writeLogLine("Scanner:  " + Scanner.nextToken.toString() + " " + Scanner.nextName);
	else if (Scanner.nextToken == numberToken)
            writeLogLine("Scanner:  " + Scanner.nextToken.toString() + " " + Scanner.nextNum);
        else
	    writeLogLine("Scanner:  " + Scanner.nextToken.toString());
    }
